include { QC_AND_SCANPY            } from '../subworkflows/qc_and_scanpy'
...
workflow SCANPY_PIPELINE {
    ...
    main:
    // ── Step 1: Create AnnData object (always runs) ─────────────────────────
    QC_AND_SCANPY(
        ch_input,
        params.min_cells,
        params.min_features,
    ...
    if (params.run_downsample == "true") {
        DOWNSAMPLE_WF(
            QC_AND_SCANPY.out.anndata,
            params.downsample_type,
            params.downsample_value
        )
        ch_s2_input = DOWNSAMPLE_WF.out.anndata
    } else {
        ch_s2_input = QC_AND_SCANPY.out.anndata
    }
    ...
    if (params.run_s2 == "true" && params.ambient_method != "none") {
        ...
        ch_s3_input = AMBIENT_CORRECTION.out.anndata
    } else {
    ...
    if (params.run_s3 == "true") {
        ...
        ch_s4_input  = FILTER_CELLS_WF.out.anndata
    ...
    if (params.run_s4 == "true") {
        ...
        ch_s5_input = DOUBLET_DETECTION_WF.out.anndata
    } else {
    ...
    if (params.run_s5 == "true") {
        ...
        ch_s6_input = CELL_CYCLE_SCORING_WF.out.anndata
    } else {
    ...
    if (params.run_s6 == "true") {
        ...
        ch_s7_input = CELL_CYCLE_SCORING_S6_WF.out.anndata
    } else {
    ...
    if (params.run_s7 == "true") {
        ...
        ch_s8_input = REGRESS_OUT_WF.out.anndata
    } else {
    ...
    if (params.run_s8 == "true") {
        ...
        ch_final = UMAP_CLUSTERING_WF.out.anndata
    } else {
    ...
    if (params.run_s8a == "true") {
        RENDER_REPORT(
            ch_final,
            file("${projectDir}/report/render_report.py"),
            file("${projectDir}/src/helper_functions.py"),
            file("${projectDir}/src/cc_genes.py")
        )
    }
