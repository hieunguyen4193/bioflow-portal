...
input.assay <- params$input.assay
...
if (file.exists(file.path(path.to.save.output, "DE_cluster_marker_genes.rds")) == FALSE){
  DefaultAssay(s.obj) <- input.assay
  if (input.assay == "SCT") s.obj <- PrepSCTFindMarkers(s.obj)
  cluster.markers <- FindAllMarkers(object = s.obj, assay = input.assay, test.use = "wilcox", slot = "data", min.pct = 0.5)
